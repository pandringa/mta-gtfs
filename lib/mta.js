...
/**
 * Gets locations of MTA trains
 * @param  {String}       feedId    optional
 * @return {Object}       schedule
 */
Mta.prototype.positions = function (feedId) {
  var options = _.pick(this.options, [ 'feed_id', 'key' ]);
  ...
  if (!this.options.key) {
    throw new Error('positions method requires MTA API key');
  }
  ...
  .then(parsers.gtfs)
  .then(function(data) {
    const updates = data.entity.filter(d => d.trip_update).map(d => ({ msg_id: d.id, ...d.trip_update }));
    const vehicles = data.entity.filter(d => d.vehicle).map(d => ({ msg_id: d.id, ...d.vehicle }));

    for(const v of vehicles){
      if(!v.trip['.nyct_trip_descriptor'].is_assigned) continue;
      const tId = v.trip['.nyct_trip_descriptor'].train_id;

      if(!trains[tId]) trains[tId] = {};
      trains[tId].vehicle = v;
    }
    for(const u of updates){
      if(!u.trip['.nyct_trip_descriptor'].is_assigned) continue;
      const tId = u.trip['.nyct_trip_descriptor'].train_id;

      if(!trains[tId]) continue;
      trains[tId].update = u;
    }

    return {
      positions: Object.entries(trains).map(([train_id,data]) => ({train_id, ...data})),
      updatedOn: data.header.timestamp.low
    };
